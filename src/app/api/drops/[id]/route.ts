...
export async function GET(request: NextRequest, { params }: RouteContext) {
  try {
    ...
    if (!isOwner) {
      await updateDropStats(id, { views: (drop.stats.views || 0) + 1 })
    }
...
export async function DELETE(request: NextRequest, { params }: RouteContext) {
  try {
    console.log('DELETE /api/drops/[id] called for:', params.id)
    ...
    try {
      console.log('Deleting files for drop:', id)
      await deleteDropFiles(id)
    } catch (storageError) {
    ...
    console.log('✅ Drop deleted successfully:', id)
...
export async function PATCH(request: NextRequest, { params }: RouteContext) {
  try {
    console.log('PATCH /api/drops/[id] called for:', params.id)
    ...
    await updateDrop(id, updateData)
    
    console.log('✅ Drop updated successfully:', id)
    const response = NextResponse.json({ 
      success: true,
